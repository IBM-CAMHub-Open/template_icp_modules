...
if [[ $1 == "2.1.0.3-fp1" ]] ; then
    if docker run -e LICENSE=accept -t --net=host -v "$(pwd)":/installer/cluster $DOCKER_REPO:2.1.0.3-ee ./cluster/ibm-cloud-private-2.1.0.3-fp1.sh -- rollback ; then
    ...
    if docker run -e LICENSE=accept --net=host --rm -t -v "$(pwd)":/installer/cluster $DOCKER_REPO:$1-ee rollback-mgtsvc ; then
        ...
        docker run -e LICENSE=accept --net=host -v /usr/local/bin:/data $DOCKER_REPO:$1-ee cp /usr/local/bin/kubectl /data
        ...
        if docker run -e LICENSE=accept --net=host --rm -t -v "$(pwd)":/installer/cluster $DOCKER_REPO:$1-ee upgrade-mgtsvc ; then
            ...
            docker run -v $(pwd):/data -e LICENSE=accept $DOCKER_REPO:2.1.0.3-ee cp /usr/local/bin/helm /data
        ...
        if docker run -e LICENSE=accept --net=host --rm -t -v "$(pwd)":/installer/cluster $DOCKER_REPO:$1-ee rollback-k8s ; then
            printf "\033[32m[*] Rollback ICP Version Succeeded \033[0m\n"
        else
