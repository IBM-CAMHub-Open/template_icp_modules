...
if [[ $1 == "2.1.0.3-fp1" ]] ; then
    ...
    if sudo docker run -e LICENSE=accept --net=host --rm -t -v "$(pwd)":/installer/cluster $DOCKER_REPO:$1-ee rollback-mgtsvc ; then
        ...
        sudo docker run -e LICENSE=accept --net=host -v /usr/local/bin:/data $DOCKER_REPO:$1-ee cp /usr/local/bin/kubectl /data
        ...
        if sudo docker run -e LICENSE=accept --net=host --rm -t -v "$(pwd)":/installer/cluster $DOCKER_REPO:$1-ee rollback-k8s ; then
            printf "\033[32m[*] Rollback ICP Version Succeeded \033[0m\n"
        else
