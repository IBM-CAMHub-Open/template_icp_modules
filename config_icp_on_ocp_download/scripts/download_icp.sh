...
if [ -n $PARAM_ICP ]; then
  ...
  if [[ $PARAM_ICP =~ $URL_REGEX ]]; then
    if [[ -e "/opt/ibm-cloud-private-rhos-${PARAM_ICP_VERSION}/${ICP_INSTALLER_FILE_NAME}" ]]; then
      printf "\033[32m[*] Previous Downloaded successful of /opt/ibm-cloud-private-rhos-${PARAM_ICP_VERSION}/${ICP_INSTALLER_FILE_NAME}\033[0m\n"
    else
      echo "[*] ICP TAR File URL was provided: $PARAM_ICP"
      DOCKER_URL=$PARAM_ICP
      ICP_PATH=/opt/ibm-cloud-private-rhos-${PARAM_ICP_VERSION}
      ...
      chmod +x /opt/ibm-cloud-private-rhos-${PARAM_ICP_VERSION}/${ICP_INSTALLER_FILE_NAME}
      printf "\033[32m[ ] Loading TAR file into Docker images....!\033[0m\n"
      tar xf  /opt/ibm-cloud-private-rhos-${PARAM_ICP_VERSION}/${ICP_INSTALLER_FILE_NAME} -O | sudo docker load
      ...
      cd /opt/ibm-cloud-private-rhos-${PARAM_ICP_VERSION}
      # sudo docker run -v $(pwd):/data -e LICENSE=accept $DOCKER_REPO:${PARAM_ICP_VERSION}-ee cp -r cluster /data
      # sudo chown ${PARAM_SSH_USER} -R /opt/ibm-cloud-private-rhos-${PARAM_ICP_VERSION}/cluster
      mkdir -p ./cluster/images; mv ${ICP_INSTALLER_FILE_NAME} /opt/ibm-cloud-private-rhos-${PARAM_ICP_VERSION}/cluster/images/
    fi 
