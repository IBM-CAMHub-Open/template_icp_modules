...
resource "null_resource" "setup_installer" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "if [ \"${var.enable_metering}\" = \"false\" ]; then sed -i '/management_services/a\\ \\ metering: disabled' /root/ibm-cloud-private-x86_64-${var.icp_version}/cluster/config.yaml; fi",
      "sed -i 's/# cluster_vip.*/cluster_vip: ${var.cluster_vip}/g' ~/ibm-cloud-private-x86_64-${var.icp_version}/cluster/config.yaml",
      "sed -i 's/# vip_iface.*/vip_iface: ${var.cluster_vip_iface}/g' ~/ibm-cloud-private-x86_64-${var.icp_version}/cluster/config.yaml",
      "sed -i 's/# proxy_vip_iface.*/proxy_vip_iface: ${var.proxy_vip_iface}/g' ~/ibm-cloud-private-x86_64-${var.icp_version}/cluster/config.yaml",
      "sed -i 's/# proxy_vip.*/proxy_vip: ${var.proxy_vip}/g' ~/ibm-cloud-private-x86_64-${var.icp_version}/cluster/config.yaml",
      ...
      "sudo docker login -u token -p ${var.bluemix_token} registry.ng.bluemix.net",
      "sudo export DOCKER_REPO=`sudo docker images |grep inception |grep ${var.icp_version} |awk '{print $1}'`",
      "bash -c 'sudo docker run --net=host -t -e LICENSE=accept  -v $(pwd):/installer/cluster ${DOCKER_REPO}:${var.icp_version} install | tee /tmp/installlog; test $${PIPESTATUS[0]} -eq 0'",
    ]
  }
}
...
resource "null_resource" "setup_installer_tar" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "if [ \"${var.enable_glusterFS}\" = \"true\" ]; then sed -i 's/storage-glusterfs: disabled/storage-glusterfs: enabled/g' ~/ibm-cloud-private-x86_64-${var.icp_version}/cluster/config.yaml; fi",
      "if [ \"${var.enable_metering}\" = \"false\" ]; then sed -i '/management_services/a\\ \\ metering: disabled' /root/ibm-cloud-private-x86_64-${var.icp_version}/cluster/config.yaml; fi",
      "if [ \"${var.enable_monitoring}\" = \"false\" ]; then sed -i '/management_services/a\\ \\ monitoring: disabled' /root/ibm-cloud-private-x86_64-${var.icp_version}/cluster/config.yaml; fi",      
      "sed -i 's/default_admin_user.*/default_admin_user: ${var.icp_admin_user}/g' ~/ibm-cloud-private-x86_64-${var.icp_version}/cluster/config.yaml",
      "sed -i 's/default_admin_password.*/default_admin_password: ${var.icp_admin_password}/g' ~/ibm-cloud-private-x86_64-${var.icp_version}/cluster/config.yaml",
