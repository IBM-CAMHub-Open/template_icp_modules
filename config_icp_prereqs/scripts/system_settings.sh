...
if [ `sysctl -n vm.max_map_count` -lt $MAX_MAP_COUNT ];then
  ...
  sudo sysctl -q -w vm.max_map_count=$MAX_MAP_COUNT
  if [ `cat /etc/sysctl.conf | grep vm.max_map_count | wc -l` -gt 0 ];then
    ...
    sudo sed -i '/vm.max_map_count/s/^/#/g' /etc/sysctl.conf
...
if [ $OS_PORT_RANGE_MIN -gt $LOCAL_PORT_RANGE_MIN ];then
  ...
  sudo sysctl -w net.ipv4.ip_local_port_range="$LOCAL_PORT_RANGE_MIN $LOCAL_PORT_RANGE_MAX"
  if [ `cat /etc/sysctl.conf | grep net.ipv4.ip_local_port_range | wc -l` -gt 0 ];then
    ...
    sudo sed -i '/net.ipv4.ip_local_port_range/s/^/#/g' /etc/sysctl.conf
    echo "net.ipv4.ip_local_port_range=\"$LOCAL_PORT_RANGE_MIN $LOCAL_PORT_RANGE_MAX\"" | sudo tee -a /etc/sysctl.conf
  else
