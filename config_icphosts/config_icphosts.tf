...
resource "null_resource" "generate_icp_hosts_file_mgmt_true" {
  ...
  provisioner "local-exec" {
    command = "bash -c '/tmp/${var.random}/generate_icp_hosts.sh -r ${var.random} -m ${var.master_public_ips} -n ${var.management_public_ips} -p ${var.proxy_public_ips} -w ${var.worker_public_ips} '"
  }
}
...
resource "null_resource" "generate_icp_hosts_file_va_true" {
  ...
  provisioner "local-exec" {
    command = "bash -c '/tmp/${var.random}/generate_icp_hosts.sh -r ${var.random} -m ${var.master_public_ips} -p ${var.proxy_public_ips} -w ${var.worker_public_ips} -v ${var.va_public_ips}'"
  }
}
...
resource "null_resource" "generate_icp_hosts_file_both_false" {
  ...
  provisioner "local-exec" {
    command = "bash -c '/tmp/${var.random}/generate_icp_hosts.sh -r ${var.random} -m ${var.master_public_ips} -p ${var.proxy_public_ips} -w ${var.worker_public_ips}'"
  }
}
